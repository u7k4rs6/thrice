...
/* Newsprint texture.
   ...
   self-contained: a fine fibre grain and a finer-grained stock variation.
   Painted at z-index 0 under .wrap (z-index 1), so it textures the paper and
   never sits on top of the type or the specimen screenshots. Blend is multiply
   against the paper colour rather than a grey wash, so ink contrast is
   unchanged.
   Tuning note: the mottle started at baseFrequency 0.012 over 3 octaves at .55
   opacity and read as water stains rather than paper. Newsprint variation is
...
input[name=f]{position:absolute;opacity:0;pointer-events:none;width:0;height:0;margin:0}
